...
@property (copy, nonatomic) void(^updateCompletion)(void);
...
- (void)updateReadingBook:(YBookDetailModel *)bookM completion:(void (^)(void))completion failure:(void (^)(NSString *))failure{
...
- (void)updateBookSummary:(YBookSummaryModel *)summaryM completion:(void (^)(void))completion failure:(void (^)(NSString *))failure {
    if (self.updateCompletion || self.updateFailure) {
        self.updateFailure(@"更新书籍错误");
        return;
    }
...
- (void)getChapterContentWith:(NSUInteger)chapter completion:(void (^)(void))completion failure:(void (^)(NSString *))failure {
    if (chapter >= self.chaptersArr.count) {
        failure(@"章节错误");
        return;
    }
