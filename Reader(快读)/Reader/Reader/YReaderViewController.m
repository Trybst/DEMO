...
- (void)showAlertView {
    if (_isFirstRead) {
        // 提示 是否将本书加入书架 好的 不了
        UIAlertView *altV = [[UIAlertView alloc]initWithTitle:@"亲～是否将本书加入书架" message:nil delegate:self cancelButtonTitle:nil otherButtonTitles:@"好的", @"不了", nil];
        [altV show];
    } else {
        
        if ((random() % 7) == 1) {
            NSDictionary *dict = [[NSDictionary alloc] initWithObjectsAndKeys:@"showAD", @"notic", nil];
            NSNotification *notification =[NSNotification notificationWithName:@"tongzhi" object:nil userInfo:dict];
            [[NSNotificationCenter defaultCenter] postNotification:notification];
        }
        
        [self dismissViewControllerAnimated:YES completion:^{
            [self.readerManager updateReadingChapter:self.chapter page:self.page];
            [self.readerManager closeReadingBook];
        }];
    }
}
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (buttonIndex == 0) {
        [[YSQLiteManager shareManager] addUserBooksWith:self.readingBook];
        [self dismissViewControllerAnimated:YES completion:^{
            [self.readerManager updateReadingChapter:self.chapter page:self.page];
            [self.readerManager closeReadingBook];
        }];
    } else {
        [[YSQLiteManager shareManager] removeUserBookWith:self.readingBook];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [[UIApplication sharedApplication] setStatusBarStyle:IS_iPhoneX ? UIStatusBarStyleDefault : UIStatusBarStyleLightContent animated:NO];
}
...
- (YDirectoryViewController *)directoryVC {
    ...
    _directoryVC.bookTitle = self.readingBook.title;
    return _directoryVC;
}
...
- (YSummaryViewController *)summaryVC {
    ...
    _summaryVC.title = self.readingBook.title;
    return _summaryVC;
}
