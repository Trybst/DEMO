...
- (id)parsingResponseObject:(id)response type:(YAPIType)type{
    ...
    if (type == YAPITypeAutoCompletion) {
        key = @"keywords";
    } else if (type == YAPITypeFuzzySearch || type == YAPITypeRecommendBook || type == YAPITypeTagBookList || type == YAPITypeAuthorBookList || type == YAPITypeCatsBookList) {
        key = @"books";
    ...
    for (NSInteger i = 0 ; i < arr.count; i++) {
        ...
        if (type == YAPITypeFuzzySearch || type == YAPITypeTagBookList || type == YAPITypeAuthorBookList || type == YAPITypeCatsBookList) {
