...
@property (weak, nonatomic) IBOutlet UIButton *addUpdateBtn;

@property (weak, nonatomic) IBOutlet UIView *adBannner;
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if ([[YSQLiteManager shareManager] isAddedBookListWith:self.bookDetail]) {
        [_addUpdateBtn setTitle:@"- 不追了" forState:UIControlStateNormal];
        [_addUpdateBtn setBackgroundColor:[UIColor lightGrayColor]];
    } else {
        [_addUpdateBtn setTitle:@"+ 追更新" forState:UIControlStateNormal];
        [_addUpdateBtn setBackgroundColor:kMBColor(206, 38, 37, 1)];
    }
}
...
- (void)setupUI {
    ...
    //    [self.tagCollectionView setContentInset:UIEdgeInsetsMake(0, 33, 0, 0)];
...
- (IBAction)startReading:(id)sender {
    ...
    if(![[YSQLiteManager shareManager] isAddedBookListWith:self.bookDetail]) {
        readerVC.isFirstRead = YES;
    } else {
        readerVC.isFirstRead = NO;
    }
...
- (IBAction)addUpdate:(id)sender {
    ...
    if (![[YSQLiteManager shareManager] isAddedBookListWith:self.bookDetail]) {
        ...
        [btn setTitle:@"- 不追了" forState:UIControlStateNormal];
        [btn setBackgroundColor:[UIColor lightGrayColor]];
        
        [YProgressHUD showErrorHUDWith:@"已添加到书架"];
    } else {
        [[YSQLiteManager shareManager] removeUserBookWith:self.bookDetail];
        [btn setTitle:@"+ 追更新" forState:UIControlStateNormal];
        [btn setBackgroundColor:kMBColor(206, 38, 37, 1)];
        
        [YProgressHUD showErrorHUDWith:@"已从书架移除"];
    }
}
...
- (void)refleshView {
    ...
    NSString *pars = _authorLabel.text;
    [_authorLabel addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithActionBlock:^(id  _Nonnull sender) {
//        NSLog(@"点击作者按钮  \"作者书单\"");
        
        YBookRecommendListViewController *vc = [UIStoryboard storyboardWithName:@"YBookRecommendListViewController" bundle:[NSBundle mainBundle]].instantiateInitialViewController;
        vc.type = YAPITypeAuthorBookList;
        vc.pars = pars;
    ...
    if ([collectionView isEqual:self.recommendCollectionView] && indexPath.section == 0 && [kind isEqualToString:UICollectionElementKindSectionHeader]) {
        ...
        UILabel *rightLabel = ({
            ...
            [label addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithActionBlock:^(id  _Nonnull sender) {
                ...
                YBookRecommendListViewController *vc = [UIStoryboard storyboardWithName:@"YBookRecommendListViewController" bundle:[NSBundle mainBundle]].instantiateInitialViewController;
                vc.type = YAPITypeRecommendBook;
                vc.pars = _selectBook.idField;
                [self.navigationController pushViewController:vc animated:YES];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    } else if ([collectionView isEqual:self.tagCollectionView]) {
        
        NSString *pars = _bookDetail.tags[indexPath.row];
        YBookRecommendListViewController *vc = [UIStoryboard storyboardWithName:@"YBookRecommendListViewController" bundle:[NSBundle mainBundle]].instantiateInitialViewController;
        vc.type = YAPITypeTagBookList;
        vc.pars = pars;
        [self.navigationController pushViewController:vc animated:YES];
