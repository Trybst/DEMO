...
#define FOREGROUND_STAR_IMAGE_NAME @"bd_star_filled"
#define BACKGROUND_STAR_IMAGE_NAME @"bd_star_empty"
...
- (void)setHiddenUncheckStar:(BOOL)hiddenUncheckStar {
    _hiddenUncheckStar = hiddenUncheckStar;
    if (self.backgroundStarView) {
        [self.backgroundStarView removeAllSubviews];
        [self.foregroundStarView removeAllSubviews];
...
- (UIView *)createStarViewWithImage:(NSString *)imageName {
    ...
    NSInteger count = self.scorePercent * self.numberOfStars;
    CGFloat imgW = self.bounds.size.width / self.numberOfStars;
    CGFloat space = imgW * 0.2;
    imgW -= space;
    for (NSInteger i = 0; i < count; i ++)
    {
        UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:imageName]];
        imageView.frame = CGRectMake(i * (imgW + space), 0, imgW, self.bounds.size.height);
