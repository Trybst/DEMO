...
#define kScreenHeightMenu [UIScreen mainScreen].bounds.size.height
#define kScreenWidthMenu [UIScreen mainScreen].bounds.size.width
...
- (void)setUpUI{
    ...
    UIView *backView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidthMenu, kScreenHeightMenu)];
...
- (void)displayAtPoint:(CGPoint)point{
    ...
    }else if (point.x >= kScreenWidthMenu - kMargin - kRadius - kTriangleHeight * 0.7){
    ...
    if (CGRectGetMaxY(self.frame) > kScreenHeightMenu) {
    ...
    if (self.max_Y> kScreenHeightMenu) {
...
- (void)adjustPosition:(CGPoint)point{
    ...
    if (self.x < kMargin) {
        self.x = kMargin;
    }else if (self.x > kScreenWidthMenu - kMargin - self.width){
        self.x = kScreenWidthMenu - kMargin - self.width;
    }
    self.layer.affineTransform = CGAffineTransformMakeScale(1.0, 1.0);
}
